...
Revision ID: ae1c41b777a9
Revises: 1b0dbab7e0c5
Create Date: 2017-10-17 14:02:42.112169
...
revision = 'ae1c41b777a9'
down_revision = '1b0dbab7e0c5'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    try:
        ...
        action_group_failback = ActionGroup('MANAGE_FAILBACKS', 1100)
        action_manage_failback = Action(
            'MANAGE_FAILBACK', type_customer, action_group_failback,
            order=1110)
        session.add(action_manage_failback)
        session.commit()
        ...
        try:
            ...
            role_superadmin.assign_action(action_manage_failback)
...
def downgrade():
    ...
    session = Session(bind=bind)
    try:
        action_manage_failback = session.query(Action).filter_by(
            name='MANAGE_FAILBACK').one_or_none()
        action_group_failbacks = session.query(ActionGroup).filter_by(
            name='MANAGE_FAILBACKS').one_or_none()
        session.delete(action_manage_failback)
        session.delete(action_group_failbacks)
        session.commit()
    finally:
        session.close()
